...
Page({
   ...
   */
  data: {
    scaleAnimation:{},         //购物车数量变化动画
    attrData: [],                            //初始化属性数组，也是wxml循环的属性数据，由attrArr初始化获得
    /**
  * {
  * attrKey:'颜色'，
  * selectedValue:'红色'                 选中的属性值
  * seletedId:'12',                      选中的属性值id
  * attrControl:[true,true,true],        属性值是否可选(是否禁用属性)
  * attrValue:['红色','白色','绿色'],     属性值
  * attrId:['12','13','14'],             属性值id
  * attrStatus:[true,false,false]            属性值选择状态
  * }
  */
    stock: 0,                                //产品库存或者当前属性下的库存
    price: 0,                                 //产品价格或者当前属性下的价格
    count: 1                                   //产品数量
  },
  ...
  scaleAnimation:function(){
    ...
    productData.cart_count = Number(productData.cart_count) + 1;
    this.setData({
      scaleAnimation: animation.export(),
  ...
  getProductInfo:function(pid){
    ...
    app.POST({
      ...
      data:{
        ...
        request:'private.product.product.detail.get',
      ...
      success:function(res){
        ...
        if(res.data.code == 0){
          ...
          that.setData({
            ...
            attrArr: res.data.data.attr_all_detail,
  ...
  */
  ...
  selectAttr: function (e) {
    ...
    if (selectedAttr.selected_id.length == attrData.length) {
      ...
      this.setData({
        price: this.data.productData.data.product_price,
        stock: this.data.productData.data.product_stock
      })
    }
    this.setData({
      attrData: attrData,
  ...
  getRecommendproduct:function(){
    ...
    app.POST({
      ...
      success:function(res){
        ...
        if(res.data.code == 0){
          that.setData({
            recommendProducts:res.data.data.data
          })
        }else{
          wx.showToast({
            title: res.data.msg,
